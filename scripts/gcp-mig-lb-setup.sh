...
# After you create a custom image from a working VM, set e.g.:
#   GOLDEN_IMAGE="projects/${PROJECT_ID}/global/images/eca-cloud-v1"
GOLDEN_IMAGE=""
...
if [[ "${CREATE_LB}" != "1" ]]; then
  ...
  echo ""
  echo "Manual gcloud (after GOLDEN_IMAGE is set):"
  echo "  gcloud compute instance-templates create ${TEMPLATE_NAME} --region=${REGION} \\"
  echo "    --machine-type=e2-medium --image=${GOLDEN_IMAGE} \\"
...
if [[ -z "${GOLDEN_IMAGE}" ]]; then
  echo "ERROR: set GOLDEN_IMAGE to your custom image (projects/.../global/images/NAME) when CREATE_LB=1"
  exit 1
fi
